  ...
  const handleDeleteProduct = async (slug) => {
    try {
      ...
      if (result.success) {
        ...
        console.error("Gagal menghapus produk:", result.error);
        setAlertState({
          show: true,
          message: `Gagal menghapus produk: ${result.error}`,
          type: "error",
        });
  ...
  onError={(e) => { e.currentTarget.src = "/blank-image.png" }}
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-center">
          <Badge
            variant={row.original.stock > 20 ? "default" : "destructive"}
            className="text-xs"
          >
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-32">
            ...
            <Link href={`/admin/products/show/${row.original.slug}`}>
              <DropdownMenuItem>View Details</DropdownMenuItem>
            </Link>
            <DropdownMenuSeparator />
            <DeleteProductAlert
              productName={row.original.name}
              onConfirm={() => handleDeleteProduct(row.original.slug)}
            />
          </DropdownMenuContent>
        </DropdownMenu>
      ),
