...
@section('content')
    ...
    <div class="max-w-3xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-md">
        ...
        <form action="{{ route('category.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
@method('PUT')
            {{-- Name --}}
            <div class="input-area">
                ...
                <div>
                    <input id="name" name="name" type="text" value="{{ old('name', $category->name) }}"
                        class="form-control pr-9" placeholder="Category Name">
                </div>
            </div>
            ...
            <div class="multiFilePreview">
                ...
                @if ($category->image)
                      <div class="mt-2">
                          <img src="{{ asset('storage/' . $category->image) }}" alt="Current Image" class="h-24 rounded">
                      </div>
                  @endif
            </div>
